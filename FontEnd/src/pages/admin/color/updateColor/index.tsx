...
const UpdateColor: React.FC<{ id: string; handleUpdateComplete: () => void }> = ({
  ...
  const onFinish = async (values: { name: string}) => {
    try {
      setIsLoading(true);
      await mutate({ id, colors: { name: values.name } }).unwrap();
      console.log(id);
  ...
  const { data: colors, isLoading: isCategoryLoading } = useGetByIdColorQuery(id);

  useEffect(() => {
    if (colors) {
      form.setFieldsValue({ name: colors.name });
    }
  }, [colors]);
