...
import { useAddDiscountCodeToUserMutation } from '@/services/user'; // Make sure to use the correct path
...
const handleAddSale = async (discount: any) => {
  ...
  console.log('savedDiscounts:', savedDiscounts);
  console.log( orderData?.docs[0].discountCode);
  console.log(orderData?.docs,'test')
  return (
      ...
      renderItem={discount => (
        <List.Item>
          <Card title={`Mã : ${discount.code}`} extra={
            ...
            disabled={
              savedDiscounts.includes(discount._id) || (orderData?.docs.some(order => order.discountCode?.includes(discount._id)))
            }
          >
            {savedDiscounts.includes(discount._id) ? (
              'Đã lưu mã'
            ) : orderData?.docs.some(order => order.discountCode?.includes(discount._id)) ? (
